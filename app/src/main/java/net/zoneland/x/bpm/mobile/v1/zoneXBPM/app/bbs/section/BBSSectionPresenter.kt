...
class BBSSectionPresenter : BasePresenterImpl<BBSSectionContract.View>(),
    BBSSectionContract.Presenter {

    override fun canPublishSubject(sectionId: String) {
        // 被禁言
        if (O2SDKManager.instance().isBBSMute()) {
            mView?.publishPermission(false)
            return
        }
        ...
        getBBSAssembleControlService(mView?.getContext())?.let { service ->
            service.subjectPublishableInSection(sectionId)
                ...
                .subscribe(ResponseHandler { (checkResult) ->
                    mView?.publishPermission(checkResult)
                },
            ...
            if (isCollected) {
                ...
                service.querySectionById(sectionId)
                    ...
                    .flatMap { response ->
                        ...
                        RealmDataService().saveBBSCollection(
                            BBSCollectionSectionVO(
