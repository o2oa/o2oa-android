...
/// fresh-blue 主题
///
class FreshBlue extends BaseTheme {

  FreshBlue(): super('fresh-blue', 66.0, 44.0) {
    //文字样式
  ...
  // 中心节点填充颜色
  Color rootRectColor = const Color.fromARGB(255, 115, 161, 191);
  // 中心节点文字颜色
  ...
  // 节点填充颜色
  Color secondRectColor = const Color.fromARGB(255, 238, 243, 246);
  // 节点文字颜色
  Color secondTextColor = const Color(0x8A000000);
  // 节点文字样式
  late TextStyle secondTextStyle;

  // 节点文字颜色
  Color nodeTextColor = const Color(0x8A000000);
  // 节点文字样式
  ...
  NodePaintElement sizeNode(NodeData data, int level) {
    ...
    /// 图片
    ///
    if( (data.image != null && data.image?.isNotEmpty == true) || (data.imageId != null && data.imageId?.isNotEmpty == true)) {
    ...
    if (level == 0) { // root没有设置id
      ...
      nodeWidth += lineWidth*2;
      nodeHeight += lineWidth*2;
      Rect borderRect = Rect.fromLTWH(0.0, 0.0, nodeWidth, nodeHeight);
      PaintStyle borderStyle = PaintStyle(color: lineColor, style: PaintingStyle.stroke, strokeWidth: lineWidth);
      elements[NodeElement.border] = RRectPaintElement(RRect.fromRectAndRadius(borderRect, Radius.circular(rectCircular)), borderStyle);
    }else {
